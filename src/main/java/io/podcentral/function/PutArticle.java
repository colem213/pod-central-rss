...
public class PutArticle implements RequestHandler<ServerlessInput, ServerlessOutput> {
  ...
  public ServerlessOutput handleRequest(ServerlessInput serverlessInput, Context context) {
    ...
    try {
      ...
      s3.putObject(new PutObjectRequest(ARTICLE_BUCKET_NAME, keyName,
          new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8)),
          new ObjectMetadata()));

      Map<String, AttributeValue> attributes = convert(serverlessInput.getQueryStringParameters());
      attributes.putIfAbsent(ARTICLE_TABLE_ID_NAME,
      ...
      dynamoDb.putItem(new PutItemRequest().withTableName(ARTICLE_TABLE_NAME).withItem(attributes));
      output.setStatusCode(200);
      output
          .setBody("Successfully inserted article " + attributes.get(ARTICLE_TABLE_ID_NAME).getS());
    } catch (Exception e) {
